...
void add_vectors(double vector1[],double vector2[],double vector3[],int size)
{
	...
	*/
	...
	int i;
	for (i=0;i<size;i++){
		vector3[i] = vector1[i] + vector2[i]; /* iterates and adds vectors assuming both arrays same size */
	}
...
double scalar_prod(double vector1[],double vector2[],int size)
{
	...
	int i;
	for (i=0;i<size;i++){
		prod += vector1[i]*vector2[i]; /*returns the scaler product*/
...
double norm2(double vector1[], int size)
{
    ...
    value = scalar_prod(vector1,vector1,size);
    L2 = sqrt(value); /* it is the sqrt of the value function which is the scaler product of vector1*vector1 */
